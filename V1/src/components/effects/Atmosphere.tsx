"use client";

import { useEffect, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { cn } from "@/lib/cn";

export function Atmosphere({
  flash,
  shake,
  distort,
}: {
  flash?: boolean;
  shake?: boolean;
  distort?: boolean;
}) {
  const [glitch, setGlitch] = useState(false);

  useEffect(() => {
    let timeout: number;
    const schedule = () => {
      const wait = 20000 + Math.random() * 20000;
      timeout = window.setTimeout(() => {
        setGlitch(true);
        window.setTimeout(() => setGlitch(false), 280);
        schedule();
      }, wait);
    };
    schedule();
    return () => window.clearTimeout(timeout);
  }, []);

  return (
    <div
      className={cn(
        "pointer-events-none fixed inset-0 z-[80]",
        shake && "animate-cam-shake",
        distort && "animate-lens-distort",
      )}
      aria-hidden
    >
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_35%,rgba(0,0,0,0.72)_100%)]" />
      <div className="scanlines absolute inset-0 opacity-40" />
      <div className="noise absolute inset-0 opacity-[0.045]" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(252,238,10,0.05),transparent_35%),radial-gradient(circle_at_80%_70%,rgba(0,229,255,0.05),transparent_40%)]" />

      <AnimatePresence>
        {glitch && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.55 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 mix-blend-screen"
            style={{
              background:
                "linear-gradient(90deg, rgba(255,23,68,0.15), transparent 40%, rgba(0,229,255,0.15))",
              transform: "translateX(-2px)",
            }}
          />
        )}
      </AnimatePresence>

      <AnimatePresence>
        {flash && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: [0, 0.85, 0] }}
            exit={{ opacity: 0 }}
            transition={{ duration: 0.55 }}
            className="absolute inset-0 bg-white"
          />
        )}
      </AnimatePresence>
    </div>
  );
}
